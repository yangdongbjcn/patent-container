...
class Container(object):
    ...
    def getHistogramFrom(self, frame, key):
        histo_table = YdbjAlgorithm().getHistogram(frame.table, key)
        return histo_table
...
class HistoChartContainer(PipeContainer):
    """ """
    def __init__(self, name, superior = None):
        """"""
        super(HistoChartContainer, self).__init__(name, superior)
    ...
    def computeHisto(self, key):
        """"""
        histo_table = YdbjAlgorithm().getHistogram(self.frame.table, key)
...
class TreeContainerIO(object):
    ...
    def addTreeData(self, cont, path):
        ...
        for file in sheet_files:
            ...
            tree_path = self.transTreePath(file_name)
        ...
        for file in sheet_files:
            ...
            file_path = path + '\\' + file
            leaf.frame.table = YdbjSheet().loadSheet(file_path)
        return cont
    ...
    def transTreePath(self, file_name):
        tree_path = file_name.replace('-', ' ')
        return tree_path
...
class TreeContainer(Container):
    ...
    def addMember(self, name, obj=None):
        if obj and not isinstance(obj, TreeContainer):  # YDBJ
            raise ValueError('member is not a Container')
        if obj is None:
            obj = TreeContainer(name)  # YDBJ
        obj.superior = self
