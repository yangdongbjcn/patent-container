$(document).ready(function(){
	...
	g_var.q_url_params = yd_url_get_params('node_brief.php');
	...
	for (var key in l_data) {
		...
		if (key == 't_applicant_std') {
			...
			var column_defs = new YdColumnDefs().init()
		        ...
		        .addButton(3, "浏览专利", base_href2, ["t_applicant_std"]).get();
			tf_datatables(t_dicts, key, column_dict, column_defs);	
			continue;
		}

		t_div = new YdHtml().init().addChartDiv(key).get();
		$("#chart_div").append($(t_div));
