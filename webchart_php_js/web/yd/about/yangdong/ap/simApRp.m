function idx = simApRp(S, rep)
    % 'sim' means 'similarity as input' and 'simple implement of ap'
    % Copyright (c) B.J. Frey & D. Dueck (2006).
    %
...
function Rp = dyRp(Rp, rep)
	rep = reshape(rep, length(rep), 1);
	rep_n = repmat(rep, 1, length(rep));
	Rp = Rp .* rep_n;
end
